...
public class MainActivity extends Activity {
    ...
    String copyReceiptToPrivate(Uri uri,String ref)throws Exception{File dir=new File(getFilesDir(),"receipts");if(!dir.exists())dir.mkdirs();File out=new File(dir,ref.replace(":","-")+".jpg");InputStream in=getContentResolver().openInputStream(uri);if(in==null)throw new Exception("Could not open receipt");OutputStream os=new FileOutputStream(out);byte[] buf=new byte[8192];int n;while((n=in.read(buf))>0)os.write(buf,0,n);in.close();os.close();return out.getAbsolutePath();}
    ...
    String uploadReceiptFile(String token,JSONObject o,String year)throws Exception{String rec=o.optString("receipt","");if(rec.isEmpty())return "";File f=new File(rec);InputStream in;if(f.exists())in=new FileInputStream(f);else in=getContentResolver().openInputStream(Uri.parse(rec));if(in==null)return rec;ByteArrayOutputStream baos=new ByteArrayOutputStream();byte[] buf=new byte[8192];int n;while((n=in.read(buf))>0)baos.write(buf,0,n);in.close();String folder=getOrCreateReceiptFolder(token,year);String boundary="receiptBoundary"+System.currentTimeMillis();String fileName=o.optString("ref","receipt")+".jpg";JSONObject meta=new JSONObject().put("name",fileName).put("parents",new JSONArray().put(folder));String start="--"+boundary+"\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n"+meta.toString()+"\r\n--"+boundary+"\r\nContent-Type: image/jpeg\r\n\r\n";String end="\r\n--"+boundary+"--";HttpURLConnection c=(HttpURLConnection)new URL("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,webViewLink").openConnection();c.setRequestMethod("POST");c.setRequestProperty("Authorization","Bearer "+token);c.setRequestProperty("Content-Type","multipart/related; boundary="+boundary);c.setDoOutput(true);OutputStream os=c.getOutputStream();os.write(start.getBytes("UTF-8"));os.write(baos.toByteArray());os.write(end.getBytes("UTF-8"));os.close();int code=c.getResponseCode();String res=readAll(code>=200&&code<300?c.getInputStream():c.getErrorStream());if(code<200||code>=300)throw new Exception("Receipt upload HTTP "+code+" "+res);JSONObject jo=new JSONObject(res);return jo.optString("webViewLink","https://drive.google.com/file/d/"+jo.getString("id")+"/view");}
